...
\subsect{Prompt 1 --- Initial project scaffold}

\textbf{Prompt:} \textit{``Read this assignment PDF and help me build a LangGraph-based CSV question-answering agent in Python using OpenAI. I need agent.py, nodes.py, and test\_agent.py with the required run\_agent() signature.''}

\textbf{Goal:} Generate the initial three-file project structure with a working LangGraph StateGraph, the four node functions (codegen, execute, evaluate, respond), and the required function signature.

\textbf{Response:} Helpful. The assistant produced all three files with correct LangGraph syntax, a clean \texttt{AgentState} TypedDict, and proper conditional edge routing for the retry loop. The \texttt{exec(code, env)} pattern and the required return dict were implemented correctly on the first attempt.

\subsect{Prompt 2 --- Adding dataset context to improve code generation}
